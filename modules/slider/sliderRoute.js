...
router.get("/", async (req, res) => {
  ...
  if (!slider) {
    res.status(500).json({ message: "No slider found !" });
  } else res.status(200).json(slider);
});
...
router.post("/", async (req, res) => {
  ...
  if (!newSlider) {
    res.status(500).json({ message: "Slider not added" });
  } else res.status(200).send(newSlider);
});
...
router.delete("/:id", async (req, res) => {
  ...
  } else return res.status(404).json({ message: "Slider not found" });
});
...
router.get("/:id", async (req, res) => {
  ...
  } else res.status(200).send(slider);
});
